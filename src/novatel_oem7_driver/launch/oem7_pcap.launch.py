...
def generate_launch_description():
    ...
    node = Node(
        ...
        parameters=[
                    ...
                    'oem7_if'         : 'Oem7ReceiverPcap',
                    'oem7_pcap_file'  : LaunchConfiguration('oem7_pcap_file'),
    ...
    pcap_arg = arg('oem7_pcap_file', '',               'Path to PCAP file to replay')
    ip_arg   = arg('oem7_ip_addr',   '10.42.7.60',     'IP Address to filter (optional)')
    port_arg = arg('oem7_port',      '3001',           'TCP port to filter (optional)')
    
    return LaunchDescription([pcap_arg, ip_arg, port_arg, 
                              node])
